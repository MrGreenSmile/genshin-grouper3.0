...
const ganyu_detail = {
  name: "감우",
  summary:
    "얼음과 활을 사용하는 메인 딜러.\n<em>감우</em>는 리월 월해정 소속으로, <em>응광</em>의 비서를 맡고 있다. 계획적이면서도 한 번 일을 시작하면 거의 쉬지도 않아 근면성실하다 못해, <em>감우</em>가 없으면 업무가 마비될 정도이다. 야근도 거의 매일 한다.\n혹사에 가까울 정도로 일에 매진하지만 그럼에도 버틸 수 있는 이유는, <em>감우</em>가 선인인 '기린'과의 혼혈이기 때문이다. 사실 <em>감우</em>는 <em>리월</em>이라는 도시가 탄생하기 전부터 활동하고 있었고, <em>암왕제군</em>의 충신이었는데, <em>암왕제군</em>의 지시로 <em>리월 칠성</em>의 비서일을 시작했던 것이다.\n<em>감우</em>는 스스로 오랜 세월을 살아온 선인이라 그런지, 인간사회에 적응하지 못했다. 그렇기에 더더욱 일에만 매진하고 인간 친구가 별로 없었다. 인간계에서 오랫동안 생활한 데다 스스로 혼혈이라는 생각에 선인계에도 섞이지 못했다. 하지만 산에서 선인 수행을 하던 <em>감우</em>를 찾아온 <em>여행자</em>와의 짧은 여행에서, 자신이 리월에서 어떤 사람이었는지를 깨닫게 된다. 동료뿐 아니라 칠성의 회장까지 <em>감우</em>를 존경하고 신뢰하고 있다는 것을 안 <em>감우</em>는, 결국 인간계로 복귀하게 된다.\n<em>감우</em>는 어렸을 적부터 <em>류운차풍진군</em>의 제자, 사실상 의붓딸처럼 함께 지냈다. <em>류운차풍진군</em>은 <em>감우</em>의 어린 시절을 상세히 알았는데, 문제는 <em>류운차풍진군</em>이 수다를 좋아한다는 것이다. <em>감우</em>와의 짧은 여행중, <em>여행자</em>에게 <em>감우</em>의 어린시절을 주절주절 얘기해버리기도 했다.\n기린은 채식주의이지만, <em>감우</em>의 어린 시절은 데굴데굴 굴러다닐 정도로 비만이었다고 한다. (코끼리도 채식주의..) 현재도 식탐이 많아 <em>리월</em> 최고의 식당중 하나인 <em>만민당</em>은 근처를 지나는 것도 조심한다고 한다. <em>향릉</em>이 말하길, 구석에서 조용히 먹는 <em>감우</em>가 안쓰러워 조금씩 더 퍼줬는데, 그걸 남기지 않는다고 한다.\n점심 식사 후에는 꼭 낮잠을 자는데, 한번 잠들면 잘 깨지 않는다. 하루는 점심 식사 후 볏짚에서 낮잠을 잤는데, 거기가 하필 마차 위라 마부가 그대로 운전해 가버리는 바람에 실종되는 사고도 있었다. 이후에는 잘 자리를 조심히 고르게 되었다고 한다.\n<em>류운차풍진군</em>에게 <em>신학</em>이라는 또다른 제자가 있다. <em>신학</em>은 본래는 인간이기에 인간 세계에 돌아가는 것이 옳다고 본 <em>류운차풍진군</em>은 <em>신학</em>을 심부름 삼아 리월로 보냈고, <em>감우</em>는 선배이자 언니로써 <em>신학</em>이 인간 세계에 잘 적응하도록 도와주고 있다. 여담으로 <em>신학</em>은 코난의 유미란, <em>감우</em>는 코난의 코난 역을 했던 성우가 맡았다.\n<em>암왕제군</em>과는 마신 전쟁때부터 함께했다. 선인들 중에서는 어린 편이지만, <em>암왕제군</em>의 명으로 <em>리월</em>이 만들어지고 인간이 사회를 만들 수 있게 도우라는 명에 따라 인간 세계에 남았다. 현재는 신분을 바꾼 <em>암왕제군</em>을 바로 알아본것으로 보인다.\n마른 체형에 검은색 바디스타킹 + 하이레그 + 흰색 천 포인트로 상당히 과감한 패션을 자랑한다. 뿐만 아니라 푸른 머리 미소녀 + 오피스 레이디(OL) + 목에 맨 워낭 + 수면부족 + 검은 뿔 + 식탐으로 모에 요소가 몰빵되어있어 인기가 상당히 많다. 초창기 원신이 널리 알려지게 된 공신 중 하나. 여담으로 <em>블루 아카이브</em>의 <em>아마우 아코</em>와 (성격 제외) 매우 닮았다. <em>아코</em>의 성 아마우(甘雨)도 같은 한자를 쓴다.\n<em>치치</em>가 야자우유를 야자염소에게서 구해다 달라고 했었는데, <em>감우</em>가 뿔이 달려있어서 그런지 야자염소라는 별명을 얻게 되었다. 그리고 이 뿔은 굉장히 예민하다고 한다(!).\n활 강공격(조준 공격)을 사용하는 최초의 딜러였는데, 이때문에 예뻐서 뽑아놓고 조작감이 불편해 쓰지 않는 사람도 많았다는 웃픈 전설이 있다. 그럼에도 초창기엔 <em>호두</em>와 함께 딜러 쌍두마차였다.",
  profile: {
    en_name: "甘雨, Ganyu",
    gender: "여성",
    rareness: "★★★★★",
