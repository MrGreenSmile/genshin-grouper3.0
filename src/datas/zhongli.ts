...
const zhongli_detail = {
  name: "종려",
  summary: `창과 바위를 사용하는 서포터.
  바위의 신이며, 마신 이름은 <em>모락스</em>. 가장 오래된 신 중 한 명이자 가장 강력한 존재로서 긴 역사와 굵직한 사건들의 중심에 서있었다.
  <em>종려</em>는 바위의 신이자, 원신 세계관에 등장한 인물 중 가장 나이가 많다고 여겨진다. 하지만 지금은 신의 자리에서 내려와 <em>종려</em>라는 이름으로 왕생당의 객경(외부에서 초청된 고문, 강사같은 사람)으로서 일하고 있다. 오래 산 만큼 해박하고 넓은 상식, 지식을 가지고 있으며 왕생당 이외에도 <em>종려</em>를 아는 사람이 많다. 특히 전통적인 의식을 주도했다.
  ...
  <em>종려</em>는 자신이 떠나도 인간들이 자립할 수 있을 지에 대해 확신을 갖기 위해 <em>종려</em>라는 이름으로 신분을 위장한 채 지내다가, 1년에 한 번 신탁을 받는 제사인 '청신의례' 현장에 자신의 허물을 던저 죽은 척을 했다. 이때 도와준게 <em>우인단</em>인데, 자신이 신성시하는 '계약'을 통해 일을 극비리에 진행하던 중, <em>타르탈리아</em>가 이 사실을 모른 채 일을 크게 키우게 된다. 이 과정에서 인간과 선인 사이가 가까워지고, 인간 사회는 한데 뭉쳐 마신과 대적하면서, 오히려 <em>종려</em>가 생각한 것 보다 더 큰 확신을 갖게 되었다.
  <em>종려</em>는 인간 생활을 시작한 지 얼마 되지 않았을 땐 돈을 자주 두고 다녀서 빌리는 일이 많았는데, 일이 일단락되고 나서도 익숙해지지 않았다. 또 돈을 두고오며 <em>아쉽게도 까먹었어</em>라며 너스레를 떨었다.
  ...
  여담으로, 한자를 그대로 읽으면 <em>종리(钟离)</em>지만, 어째선지 한국에서만 <em>종려</em>로 번역되었다. 종려(棕櫚)는 종려나무라는 야자수의 일종으로, 승리와 번영을 상징하며, 영화 시상식 "황금종려상"의 이름으로도 사용되었다.
  <em>종려</em>의 마신 이름인 모락스(Molax)는 솔로몬 72악마 중 하나로, 소의 머리를 한 채 왕좌를 타고 등장한다. 가나안 지역에 전해지는 신이자 악마 "몰렉"과 동일시 되며, 지옥 "게헨나"의 근원이라고 한다. 아이들을 산채로 제물로 삼아 아이들의 피와 부모들의 눈물로 몸을 씼었고, 지옥회의에 참석할 정도로 강력한 존재로 묘사된다.`,
  profile: {
    en_name: "钟离, Zhongli",
    gender: "남성",
    rareness: "★★★★★",
