...
const hutao_detail = {
  name: "호두",
  summary: `
    불 원소와 창을 사용하는 메인 딜러. HP를 소모해 대미지 증가 버프를 받는 일명 '버서커'형 캐릭터다.
    <em>호두</em>는 리월의 장례와 제례를 주관하는 '왕생당(往生堂)'의 77대 당주이다. <em>종려</em>가 이곳에서 객경(외부 자문관)으로 근무하고 있으며, 신의 허물 사건 당시, <em>종려</em>를 중심으로 떠나간 신을 기리는 송신의례를 주관했다. 해등절에서도 중심적인 역할을 하는 데, 해등절이 축제의 의미도 있지만 제령의 의미도 있기 때문이다.
    <em>호두</em>는 붙임성이 강하면서 괴짜스러운 면을 가지고 있다. 요리를 할 땐 확률적으로 '이상한' 요리가 튀어나온다(!)
    죽음에 가까운 특성 때문인지, 작은 유령을 데리고 다닌다.
    <em>호두</em>의 모자 '건곤태괘모'는 75대 당주인 그녀의 할아버지의 것이었다. 당주는 떠나갈 때, 갖고 있던 모든 미련을 불태워야 하지만, 모자만큼은 손녀에게 물려줬다. 할아버지는 풍채가 커 모자도 컸지만, <em>호두</em>는 이걸 밤낮으로 수선해 자기 머리에 맞췄다. 모자에 꽂혀있는 매화가지는 <em>호두</em>가 직접 만든 장식으로, 바람에 말린 후 색을 칠하고 기름에 담가 3일동안 말린다. 만지면 그윽한 향이 난다고 한다. 팔문칠문진의 의식을 치루기 전, 여행자에게 주며 주변정리를 했었다.
    <em>호두</em>는 독특한 성격과 귀여움, 역동적인 모션, 강력한 '버서커' 모드로 원신 초창기부터 유입을 이끌어왔다. 많은 캐릭터가 나온 지금, 편의성면에선 밀리기 시작했지만 여전히 강력한 메인 딜러로서 자리를 지키고 있다.`,
  profile: {
    en_name: "胡桃, Hutao",
    gender: "여성",
    rareness: "★★★★★",
